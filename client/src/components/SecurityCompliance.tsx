...
export default function SecurityCompliance() {
  ...
  return (
    <section id="security" className="py-12 md:py-24 px-6 bg-card border-t border-border/10">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-16">
          <h2 className="text-2xl md:text-3xl lg:text-4xl font-bold mb-4" data-testid="text-security-title">
            Security & Compliance
          </h2>
          <p className="text-lg text-muted-foreground max-w-3xl mx-auto leading-relaxed" data-testid="text-security-subtitle">
            Infrastructure designed for mental health and wellness practices with enterprise-grade security.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
          {securityFeatures.map((feature) => {
            ...
            return (
              ...
              >
                <div className={`w-10 h-10 rounded-lg ${feature.iconBg} bg-opacity-10 flex items-center justify-center mb-5`}>
                  <Icon className="w-5 h-5 text-foreground" />
                </div>
                <h3 className="text-base font-bold mb-2" data-testid={`text-security-${feature.title.toLowerCase().replace(/\s+/g, '-')}-title`}>
                  {feature.title}
                </h3>
                <p className="text-sm text-muted-foreground leading-relaxed" data-testid={`text-security-${feature.title.toLowerCase().replace(/\s+/g, '-')}-description`}>
                  {feature.description}
                </p>
              </Card>
            );
        ...
        <div className="bg-background rounded-xl p-8 md:p-10 border border-border">
          <p className="text-base text-muted-foreground leading-relaxed mb-6" data-testid="text-security-aws-statement">
          ...
          <p className="text-base text-muted-foreground leading-relaxed" data-testid="text-security-safeguards">
            We implement additional safeguards such as encryption at rest and in transit, IAM-based access control, and continuous logging through AWS CloudTrail to ensure patient data remains private and secure.
          </p>
        </div>
      </div>
    </section>
